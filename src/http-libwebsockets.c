...
static qd_log_source_t* http_log; /* For LWS messages */
...
void qd_http_listener_free(qd_http_listener_t *hl) {
    if (!hl) return;
    if (hl->listener) qd_listener_decref(hl->listener);
    free(hl);
}
                           ...
                           void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_USER: {
        ...
        (void)next_tick;
        if (next_tick > hs->now && next_tick < hs->next_tick) {
...
qd_http_server_t *qd_http_server(qd_server_t *s, qd_log_source_t *log) {
    ...
    if (hs) {
        ...
        if (hs->context) {
            hs->thread = sys_thread(thread_run, hs);
        }
        if (!hs->thread) {
