...
static void on_accept(pn_event_t *e)
{
    ...
    /* FIXME aconway 2017-01-20: restore HTTP accept */
}
...
static void on_connection_bound(qd_server_t *server, pn_event_t *e) {
    ...
    const qd_server_config_t *config = NULL; /* FIXME aconway 2017-04-11:  */
    if (ctx->listener) {        /* Accepting an incoming connection */
        ...
        {
            ctx->policy_counted = true; /* FIXME aconway 2017-04-11: why needed? */
        } else {
        ...
        if (config->ssl_profile) {
            ...
            if (!domain) {
                connect_fail(ctx, QD_AMQP_COND_INTERNAL_ERROR,
                                       "SSL domain failed for connection to %s", name);
                return;
            }
            ...
            if (config->ssl_trusted_certificate_db) {
                if (pn_ssl_domain_set_trusted_ca_db(domain, config->ssl_trusted_certificate_db)) {
                    connect_fail(ctx, QD_AMQP_COND_INTERNAL_ERROR,
                                           "SSL CA configuration failed for %s", name);
                }
            }
            ...
            if (config->ssl_require_peer_authentication) {
                ...
                if (pn_ssl_domain_set_peer_authentication(domain,
                                                          ...
                                                          trusted)) {
                    connect_fail(ctx, QD_AMQP_COND_INTERNAL_ERROR,
                                     "SSL peer auth configuration failed for %s", name);
                    return;
                }
            }
            ...
            if (config->ssl_certificate_file) {
                if (pn_ssl_domain_set_credentials(domain,
                                                  ...
                                                  config->ssl_password)) {
                    connect_fail(ctx, QD_AMQP_COND_INTERNAL_ERROR,
                                           "SSL local configuration failed for %s", name);
                    return;
                }
            }
            ...
            if (config->verify_host_name) {
                if (pn_ssl_domain_set_peer_authentication(domain, PN_SSL_VERIFY_PEER_NAME, NULL)) {
                    connect_fail(ctx, QD_AMQP_COND_UNAUTHORIZED_ACCESS,
                                           "SSL peer host name verification failed for %s", name);
                    return;
                }
            }
...
static void qd_connection_free(qd_connection_t *ctx)
{
    ...
    /* Tell the container */
    notify_closed(ctx->server->container, ctx, qd_connection_get_context(ctx));
...
static bool handle(pn_event_t *e, qd_server_t *qd_server) {
    ...
    case PN_CONNECTION_WAKE: {
        /* Wakeup requested to invoke deferred calls */
    ...
    /* FIXME aconway 2017-04-12: messy! revisit. */

    /* Now let the container handler process the event */
...
static void try_open_cb(void *context) {
    /* FIXME aconway 2017-03-27: lock too big? */
...
qd_listener_t *qd_server_listener(qd_server_t *server)
{
    ...
    li->server      = server;
    li->http = NULL;     /* FIXME aconway 2017-01-24: restore HTTP */
    return li;
}
